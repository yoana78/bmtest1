...
export const LIST_SCHEMA = {
  storyBlocks: {
    label: '홈 스토리 블록',
    page: 'home',
    note: '홈 화면에서 사진과 함께 스크롤되는 소개 블록입니다. 태그는 "01 — 철학" 처럼 번호를 함께 적어주세요.',
    itemLabel: (item) => item.titleKo || '새 블록',
    newItem: () => ({ tagKo: '', tagEn: '', titleKo: '', titleEn: '', bodyKo: '', bodyEn: '', metaKo: [], metaEn: [], image: '' }),
    fields: [
      { key: 'tag', label: '태그 (예: 01 — 철학)', type: 'text' },
      { key: 'title', label: '제목', type: 'text' },
      { key: 'body', label: '본문 (엔터로 줄바꿈)', type: 'lines' },
      { key: 'meta', label: '작은 라벨 (한 줄에 하나씩)', type: 'lines' },
  ...
  petDistributors: {
    ...
    newItem: () => ({ id: `petdist-${Date.now()}`, nameKo: '', nameEn: '', logo: '', logoScale: 1 }),
    fields: [
      { key: 'name', label: '이름', type: 'text' },
      { key: 'logo', label: '로고', type: 'image', width: 400, height: 200, fit: 'contain', scaleKey: 'logoScale' },
      { key: 'logoScale', label: '로고 표시 크기', type: 'scale' }
